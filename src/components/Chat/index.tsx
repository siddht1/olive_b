        ...
        if (store.remainingToken < 0) {
          throw new Error(
            store.sessionSettings.continuousDialogue
              ? "This conversation is too long, please clear the previous part of the conversation or shorten the current question."
              : "The question is too long, please shorten it."
          )
        }
        setStore("loading", true)
        controller = new AbortController()
        //When closing a continuous conversation, the effective context only contains the locked conversation.
        await fetchGPT(
          store.sessionSettings.continuousDialogue
            ? store.validContext
            : [
